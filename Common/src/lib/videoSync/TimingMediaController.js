...
define(function (require) {
  ...
  var TimingMediaController = function (timing, options) {
    ...
    this.removeAllMediaElement= function () {
	controlledElements = [];
	return;
    };
    ...
    var stopAmortizationPeriod = function () {
      ...
      controlledElements.forEach(function (wrappedEl) {
        ...
        // adjusted with the newly computed drift rate
          ...
          if (wrappedEl.vector.velocity > 10) {
              wrappedEl.vector.velocity = 10;
          } else if (wrappedEl.vector.velocity < 0.1) {
                wrappedEl.vector.velocity = 0.1;
              Dnv.monitor.sendLogRabbit("[TimingMediaController] Tiene velocidad menos de cero", "ERROR");
          }
        try {

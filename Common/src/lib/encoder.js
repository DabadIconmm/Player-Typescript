...
Dnv.encoder = (function () {
    ...
    /*
     * Funcionalidad de DenevaCrypto
     * El código de CryptoJs se mete en el closure para ofuscarlo algo más...
     */

    /* CONTENIDO DE CriptoJS md5.js */
...
}), v = m.enc = {}, t = v.Hex = { stringify: function (b) { var a = b.words; b = b.sigBytes; for (var g = [], j = 0; j < b; j++) { var k = a[j >>> 2] >>> 24 - 8 * (j % 4) & 255; g.push((k >>> 4).toString(16)); g.push((k & 15).toString(16)) } return g.join("") }, parse: function (b) {
...
4294967296); k[(h + 64 >>> 9 << 4) + 15] = (l << 8 | l >>> 24) & 16711935 | (l << 24 | l >>> 8) & 4278255360; k[(h + 64 >>> 9 << 4) + 14] = (b << 8 | b >>> 24) & 16711935 | (b << 24 | b >>> 8) & 4278255360; a.sigBytes = 4 * (k.length + 1); this._process(); a = this._hash; k = a.words; for (b = 0; 4 > b; b++) h = k[b], k[b] = (h << 8 | h >>> 24) & 16711935 | (h << 24 | h >>> 8) & 4278255360; return a
    ...
    /* CONTENIDO DE CriptoJS tripledes.js */
...
}), w = d.enc = {}, v = w.Hex = { stringify: function (a) { var c = a.words; a = a.sigBytes; for (var m = [], f = 0; f < a; f++) { var t = c[f >>> 2] >>> 24 - 8 * (f % 4) & 255; m.push((t >>> 4).toString(16)); m.push((t & 15).toString(16)) } return m.join("") }, parse: function (a) {
...
m)).finalize(c)
    ...
    (function () {
        var u = CryptoJS, l = u.lib.WordArray; u.enc.Base64 = { stringify: function (d) { var n = d.words, l = d.sigBytes, s = this._map; d.clamp(); d = []; for (var q = 0; q < l; q += 3) for (var w = (n[q >>> 2] >>> 24 - 8 * (q % 4) & 255) << 16 | (n[q + 1 >>> 2] >>> 24 - 8 * ((q + 1) % 4) & 255) << 8 | n[q + 2 >>> 2] >>> 24 - 8 * ((q + 2) % 4) & 255, v = 0; 4 > v && q + 0.75 * v < l; v++) d.push(s.charAt(w >>> 6 * (3 - v) & 63)); if (n = s.charAt(64)) for (; d.length % 4; ) d.push(n); return d.join("") }, parse: function (d) {
...
l)
    ...
    CryptoJS.lib.Cipher || function (u) {
        var l = CryptoJS, d = l.lib, n = d.Base, p = d.WordArray, s = d.BufferedBlockAlgorithm, q = l.enc.Base64, w = l.algo.EvpKDF, v = d.Cipher = s.extend({ cfg: n.extend(), createEncryptor: function (m, a) { return this.create(this._ENC_XFORM_MODE, m, a) }, createDecryptor: function (m, a) { return this.create(this._DEC_XFORM_MODE, m, a) }, init: function (m, a, b) { this.cfg = this.cfg.extend(b); this._xformMode = m; this._key = a; this.reset() }, reset: function () { s.reset.call(this); this._doReset() }, process: function (a) { this._append(a); return this._process() },
...
a, b, e); this._prevBlock = d
        ...
        }); b = b.CBC = r; r = (l.pad = {}).Pkcs7 = { pad: function (a, b) { for (var c = 4 * b, c = c - a.sigBytes % c, e = c << 24 | c << 16 | c << 8 | c, d = [], l = 0; l < c; l += 4) d.push(e); c = p.create(d, c); a.concat(c) }, unpad: function (a) { a.sigBytes -= a.words[a.sigBytes - 1 >>> 2] & 255 } }; d.BlockCipher = v.extend({ cfg: v.cfg.extend({ mode: b, padding: r }), reset: function () {
...
this, c && c.words)
        ...
        }); var e = d.CipherParams = n.extend({ init: function (a) { this.mixIn(a) }, toString: function (a) { return (a || this.formatter).stringify(this) } }), b = (l.format = {}).OpenSSL = { stringify: function (a) {
...
1701076831]).concat(a).concat(c) : c).toString(q)
        ...
        }, a = d.SerializableCipher = n.extend({ cfg: n.extend({ format: b }), encrypt: function (a, c, b, d) { d = this.cfg.extend(d); var l = a.createEncryptor(b, d); c = l.finalize(c); l = l.cfg; return e.create({ ciphertext: c, key: b, iv: l.iv, algorithm: a, mode: l.mode, padding: l.padding, blockSize: a.blockSize, formatter: d.format }) },
        ...
        }), l = (l.kdf = {}).OpenSSL = { execute: function (a, c, b, d) { d || (d = p.random(8)); a = w.create({ keySize: c + b }).compute(a, d); b = p.create(a.words.slice(c), 4 * b); a.sigBytes = 4 * c; return e.create({ key: a, iv: b, salt: d }) } }, c = d.PasswordBasedCipher = a.extend({ cfg: a.cfg.extend({ kdf: l }), encrypt: function (c, b, e, d) {
...
}); d.DES = n._createHelper(r); s = s.TripleDES = n.extend({ _doReset: function () {
...
r.createEncryptor(p.create(b.slice(4, 6)))
    ...
    // FIXME: Hay que buscar alguna forma de ofuscar mejor esto. ¿precalculando el md5? reemplazando "MD5", "DES" "TripeDES" por otro string?
    var pwd;
    var myIv = CryptoJS.enc.Hex.parse('0000000000000000');
    //function initPwd() {
    // La pass son los 8 primeros bytes del md5
    var p = "ICON Multimedia 2005";
    var md5 = CryptoJS.MD5(p);
    //var words  = CryptoJS.enc.Latin1.parse('Hello, World!');
    //var latin1 = CryptoJS.enc.Latin1.stringify(words);
    pwd = CryptoJS.enc.Latin1.parse(CryptoJS.enc.Latin1.stringify(md5).substr(0, 8));

    //pwd = md5.toString().substr(0, 8);
    ...
    return {
        codificar: function codificar(msg) {
            ...
            var encrypted = CryptoJS.DES.encrypt(msg, pwd, { iv: myIv });
            encrypted = encrypted.ciphertext.toString(CryptoJS.enc.Hex).toUpperCase();
            return encrypted;
        },

        descodificar: function descodificar(msg) {
            var decrypted = CryptoJS.DES.decrypt({ ciphertext: CryptoJS.enc.Hex.parse(msg) }, pwd, { iv: myIv });
            //decrypted = decrypted.toString(CryptoJS.enc.Utf8); //CryptoJS.enc.Latin1.stringify(decrypted)
            decrypted = decrypted.toString(CryptoJS.enc.Latin1); //CryptoJS.enc.Latin1.stringify(decrypted)
            return decrypted;
        }
    };
})();